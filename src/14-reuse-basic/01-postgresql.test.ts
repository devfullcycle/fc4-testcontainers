...
describe("PostgreSQL 1", () => {
  let container: StartedPostgreSqlContainer;
  let pgPool: Pool;
  let pgClient: PoolClient;

  beforeAll(async () => { // método executado uma vez antes de todos os testes
  ...
  afterAll(async () => {
    ...
    await pgPool.end();
    //await container.stop(); // Reuso do container, então não paramos aqui 20 tests vs 200 ms = 4s
  });
